...
/**
 * The committed header slices — the first four kilobytes of each sample file,
 * which is exactly what the detector reads from a dropped file.
 *
 * Only the slices are in the repository; the full media files are generated by
 * `npm run fixtures`. That keeps these tests runnable straight after a clone
 * without putting megabytes of binaries in version control.
 */
const HEADERS = path.resolve(import.meta.dirname, '../../../tests/fixtures/headers');

function head(name: string): Uint8Array {
  return new Uint8Array(
    readFileSync(path.join(HEADERS, `${name}.head`)).subarray(0, SNIFF_BYTES),
  );
}
